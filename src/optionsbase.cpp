...
OptionsBase::OptionsBase(const std::string& description, const std::string& app_name)
        : super(description, app_name)
{
...
void OptionsBase::parse(int argc, const char** argv, bool emitCmdline)
{
    ...
    {
        this->name(fs::path(argv[0]).filename().string());
    }
    try
    {
...
void OptionsBase::logHeader()
{
    ...
    {
        size_t labelWidth =
                std::accumulate(m_optionDisplayList.begin(), m_optionDisplayList.end(), 0, [](size_t maxWidth, auto& od)
                { return std::max(maxWidth, od.label.size()); });
